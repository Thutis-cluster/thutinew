...
if ('webkitSpeechRecognition' in window && micBtn) {
    ...
    speakCartContents();
        ...
        // 🔍 SEARCH (this is the FIX for Contact Us page)
        searchInput.value = spoken;
        speak(`Searching for ${spoken}`);

        setTimeout(() => {
            if (window.location.pathname.includes("contactus.html")) {
                highlightAndScroll(spoken); // ✅ NO refresh
            } else {
...
const form = document.getElementById("contactForm");
  ...
  form.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      if (result.success) {
        successMsg.textContent = "✅ Your message has been sent! Check your email for confirmation.";
