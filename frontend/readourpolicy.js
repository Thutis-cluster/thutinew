...
// ✨ Read our Policy Page Auto-Search (with multiple highlights)
document.getElementById('homepageSearchForm')?.addEventListener('submit', function (e) {
    const searchValue = searchInput?.value.trim().toLowerCase();
    if (!searchValue) return;

    // If currently on Read our Policy
    if (window.location.pathname.includes("readourpolicy.html")) {
        ...
        // Otherwise, check if Read our Policy page contains the term
        fetch('readourpolicy.html')
            .then(res => res.text())
            .then(html => {
                if (html.toLowerCase().includes(searchValue)) {
                    e.preventDefault();
                    window.location.href = `termsandconditions.html?search=${encodeURIComponent(searchValue)}`;
                }
            })
            .catch(() => {});
    }
...
// 🔍 Live Search for Read our Policy Page
document.addEventListener("DOMContentLoaded", () => {
    if (!window.location.pathname.includes("readourpolicy.html")) return;
