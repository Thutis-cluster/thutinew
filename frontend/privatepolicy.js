...
document.getElementById('homepageSearchForm')?.addEventListener('submit', function (e) {
    ...
    if (window.location.pathname.includes("privatepolicy.html")) {
        ...
        fetch('privatepolicy.html')
            .then(res => res.text())
            .then(html => {
                if (html.toLowerCase().includes(searchValue)) {
                    e.preventDefault();
                    window.location.href = `privatepolicy.html?search=${encodeURIComponent(searchValue)}`;
                }
            })
            .catch(() => {});
    }
...
document.addEventListener("DOMContentLoaded", () => {
    if (!window.location.pathname.includes("privatepolicy.html")) return;
