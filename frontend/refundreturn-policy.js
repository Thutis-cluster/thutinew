...
// ✨ Refund/Return-policy Page Auto-Search (with multiple highlights)
document.getElementById('homepageSearchForm')?.addEventListener('submit', function (e) {
    const searchValue = searchInput?.value.trim().toLowerCase();
    if (!searchValue) return;

    // If currently on Refund/Return-policy page
    if (window.location.pathname.includes("refundreturn-policy.html")) {
        ...
        // Otherwise, check if Refund/Return-policy page contains the term
        fetch('refundreturn-policy.html')
            .then(res => res.text())
            .then(html => {
                if (html.toLowerCase().includes(searchValue)) {
                    e.preventDefault();
                    window.location.href = `refundreturn-policy.html?search=${encodeURIComponent(searchValue)}`;
                }
            })
            .catch(() => {});
    }
...
// 🔍 Live Search for Refund/Return-policy Page
document.addEventListener("DOMContentLoaded", () => {
    if (!window.location.pathname.includes("refundreturn-policy.html")) return;
